...
public class LookupJoinITCase extends CatalogITCaseBase {
    ...
    //        initTable(cacheMode);
    //        String query =
    //                "SELECT T.i, D.j, D.k1, D.k2 FROM T LEFT JOIN DIM for system_time as of
    ...
    //
    //        String query =
    //                "SELECT T.i, D.j, D.k1 FROM T LEFT JOIN DIM for system_time as of T.proctime
    // AS D ON T.i = D.i";
    //        BlockingIterator<Row, Row> iterator =
    // BlockingIterator.of(sEnv.executeSql(query).collect());
    //
    ...
    //        sql("INSERT INTO DIM VALUES (1, 11, 111, 1111), (2, 22, 222, 2222), (3, 22, 333,
    // 3333)");
    //
    //        String query =
    //                "SELECT D.i, T.i, D.k1, D.k2 FROM T LEFT JOIN DIM for system_time as of
    ...
    //
    //        String query =
    //                "SELECT T.i, D.k1 FROM T LEFT JOIN DIM for system_time as of T.proctime AS D
    // ON T.i = D.j";
    //        BlockingIterator<Row, Row> iterator =
    // BlockingIterator.of(sEnv.executeSql(query).collect());
    //
    ...
    //        List<Row> result = iterator.collect(3);
    //        assertThat(result)
    //                .containsExactlyInAnyOrder(Row.of(11, null), Row.of(22, 333), Row.of(33,
    // null));
    //
    //        sql("INSERT INTO DIM VALUES (2, 44, 444, 4444), (3, 33, 333, 3333)");
    //        Thread.sleep(2000); // wait refresh
    ...
    //        assertThat(result)
    //                .containsExactlyInAnyOrder(
    //                        Row.of(11, null), Row.of(22, null), Row.of(33, 333), Row.of(44,
    // null));
    //
    //        iterator.close();
    //    }
    ...
    //        sql(
    //                "CREATE TABLE DIM2 (i INT PRIMARY KEY NOT ENFORCED, j INT, k1 INT, k2 INT)
    // WITH"
    //                        + "
    // ('merge-engine'='partial-update','continuous.discovery-interval'='1 ms')");
    //        String query =
    //                "SELECT T.i, D.j, D.k1, D.k2 FROM T LEFT JOIN DIM2 for system_time as of
    // T.proctime AS D ON T.i = D.i";
    //        assertThatThrownBy(() -> sEnv.executeSql(query))
    //                .hasRootCauseMessage(
    //                        "Partial update streaming"
    //                                + " reading is not supported. "
    //                                + "You can use 'lookup' or 'full-compaction' changelog
    // producer to support streaming reading. "
    //                                + "('input' changelog producer is also supported, but only
    ...
    //                        + " T.i, D.j, D.k1, D.k2 FROM T LEFT JOIN DIM for system_time as of
    // T.proctime AS D ON T.i = D.i";
    //        BlockingIterator<Row, Row> iterator =
    // BlockingIterator.of(sEnv.executeSql(query).collect());
    //
    ...
    public void testAsyncRetryLookup(LookupCacheMode cacheMode) throws Exception {
        ...
        String query =
                "SELECT /*+ LOOKUP('table'='D', 'retry-predicate'='lookup_miss',"
                        + " 'retry-strategy'='fixed_delay', 'output-mode'='allow_unordered', 'fixed-delay'='3s','max-attempts'='100') */"
    ...
    //        initTable(LookupCacheMode.AUTO);
    //        String query =
    //                "SELECT T.i, D.j, D.k1, D.k2 FROM T LEFT JOIN PARTITIONED_DIM for system_time
    // as of T.proctime AS D ON T.i = D.i";
    //        BlockingIterator<Row, Row> iterator =
    // BlockingIterator.of(sEnv.executeSql(query).collect());
    //
    //        sql("INSERT INTO T VALUES (1), (2), (3)");
    ...
    //                        + "PARTITIONED BY (`i`) WITH ('continuous.discovery-interval'='1
    // ms')");
    //
    //        String query =
    //                "SELECT T.i, D.j, D.k1, D.k2 FROM T LEFT JOIN DIM_NO_PK for system_time as of
    // T.proctime AS D ON T.i "
    //                        + "= D.i";
    //        BlockingIterator<Row, Row> iterator =
    // BlockingIterator.of(sEnv.executeSql(query).collect());
    //
    ...
    //
    //        sql(
    //                "INSERT INTO DIM_NO_PK VALUES (1, 11, 111, 1111), (1, 12, 112, 1112), (1, 11,
    // 111, 1111)");
    //        Thread.sleep(2000); // wait refresh
    //        sql("INSERT INTO T VALUES (1), (2), (4)");
    ...
    //        sql(
    //                "CREATE TABLE DIM_WITH_SEQUENCE (i INT PRIMARY KEY NOT ENFORCED, j INT, k1
    // INT, k2 INT) WITH"
    //                        + " ('continuous.discovery-interval'='1 ms', 'sequence.field' =
    // 'j')");
    //        sql("INSERT INTO DIM_WITH_SEQUENCE VALUES (1, 11, 111, 1111), (2, 22, 222, 2222)");
    //
    //        String query =
    //                "SELECT T.i, D.j, D.k1, D.k2 FROM T LEFT JOIN DIM_WITH_SEQUENCE for
    ...
    //        String query =
    //                "SELECT /*+ LOOKUP('table'='D', 'retry-predicate'='lookup_miss',"
    //                        + " 'retry-strategy'='fixed_delay', 'output-mode'='allow_unordered',
    // 'fixed-delay'='3s','max-attempts'='60') */"
    //                        + " T.i, D.j, D.k1, D.k2 FROM T LEFT JOIN DIM_WITH_SEQUENCE /*+
    // OPTIONS('lookup.async'='true') */ for system_time as of T.proctime AS D ON T.i = D.i";
    //        BlockingIterator<Row, Row> iterator =
    // BlockingIterator.of(sEnv.executeSql(query).collect());
    //
    //        sql("INSERT INTO T VALUES (3)");
    ...
    //        sql("INSERT INTO T VALUES (1)");
    //        assertThat(iterator.collect(2))
    //                .containsExactlyInAnyOrder(Row.of(1, 11, 111, 1111), Row.of(2, 22, 222,
    // 2222));
    //
    ...
    //                        + " 'retry-strategy'='fixed_delay', 'output-mode'='allow_unordered',
    // 'fixed-delay'='3s','max-attempts'='60') */"
    //                        + " T.i, D.i, D.j, D.k2 FROM T LEFT JOIN DIM_WITH_SEQUENCE /*+
    // OPTIONS('lookup.async'='true') */ for system_time as of T.proctime AS D ON T.i = D.k1";
    //        BlockingIterator<Row, Row> iterator =
    // BlockingIterator.of(sEnv.executeSql(query).collect());
    //
    ...
    //        assertThat(iterator.collect(3))
    //                .containsExactlyInAnyOrder(
    //                        Row.of(111, 1, 1, 1111), Row.of(111, 2, 2, 2222), Row.of(333, 3, 3,
    // 3333));
    //
    //        iterator.close();
    //    }
    ...
    //        sql(
    //                "CREATE TABLE DIM (k2 INT, k1 INT, j INT , i INT, PRIMARY KEY(i, j) NOT
    // ENFORCED) WITH"
    //                        + " ('continuous.discovery-interval'='1 ms', 'lookup.cache'='%s',
    // 'bucket' = '2', 'bucket-key' = 'j')",
    //                mode);
    //
    ...
    //
    //        String query =
    //                "SELECT T2.i, D.j, D.k1, D.k2 FROM T2 LEFT JOIN DIM for system_time as of
    // T2.proctime AS D ON T2.i = D.i and T2.j = D.j";
    //        BlockingIterator<Row, Row> iterator =
    // BlockingIterator.of(sEnv.executeSql(query).collect());
    //
