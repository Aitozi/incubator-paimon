...
/** A {@link SplitReadProvider} to read primary-key data-evolution files. */
public class DataEvolutionMergeFileSplitReadProvider implements SplitReadProvider {

    private final LazyField<SplitRead<InternalRow>> splitRead;

    public DataEvolutionMergeFileSplitReadProvider(
            Supplier<DataEvolutionMergeFileSplitRead> supplier, SplitReadConfig splitReadConfig) {
        this.splitRead =
                new LazyField<>(
                        () -> {
                            DataEvolutionMergeFileSplitRead read = supplier.get();
                            SplitRead<InternalRow> converted =
                                    SplitRead.convert(
                                            read,
                                            split ->
                                                    unwrap(
                            ...
                            splitReadConfig.config(converted);
                            return converted;
                        });
    }
